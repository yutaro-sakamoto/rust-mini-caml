...
//fn infer_type(env: &mut BTreeMap<String, Type>, exp: &mut Exp) -> Result<Type, (Exp, Type, Type)> {
//    match &mut *exp {
//        Exp::Unit => return Ok(Type::Unit),
//        Exp::Bool(_) => return Ok(Type::Bool),
//        Exp::Int(_) => return Ok(Type::Int),
//        Exp::Float(_) => return Ok(Type::Float),
//        Exp::Not(mut e) => match infer_type(env, &mut *e) {
//            Ok(infered_type) => Ok(Type::Unit),
//            Err((mut ee, mut t1, mut t2)) => Err((
//                deref_term(&mut ee),
//                deref_type(&mut t1),
//                deref_type(&mut t2),
//            )),
//        },
//        _ => unreachable!("Inferring the type of a given expression fails"), //
//    }
//}
